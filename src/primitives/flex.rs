...
// --- LOGIC ---
pub struct FlexLogic<'a> { pub children: Children<'a> }
// --- VIEW ---
pub struct FlexView { pub core: ViewCore }
// --- COMPONENT ---
pub struct Flex<'a> { pub id: String, pub logic: FlexLogic<'a>, pub view: FlexView }

impl<'a> Flex<'a> {
    pub fn new() -> Self {
        let mut style = Style::default(); Theme::current().apply_defaults(&mut style);
        style.layout.display = crate::support::Display::Flex;
        Self { id: generate_id(), logic: FlexLogic { children: Children::new() }, view: FlexView { core: ViewCore::new(style) } }
    }
    pub fn child(mut self, child: Box<dyn Component + 'a>) -> Self { self.logic.children.add(child); self.view.core.mark_dirty(); self }
}

impl<'a> Stylable for Flex<'a> { fn get_style_mut(&self) -> RwLockWriteGuard<'_, Style> { self.view.core.get_style_mut() } }
