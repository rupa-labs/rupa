...
struct RupauiRunner {
    ...
    pub taffy: TaffyTree<()>,
    pub _layout_map: HashMap<String, NodeId>,
    pub app_name: String,
    ...
    fn window_event(&mut self, event_loop: &winit::event_loop::ActiveEventLoop, _id: WindowId, event: WindowEvent) {
        match event {
            ...
            WindowEvent::CursorMoved { position, .. } => {
                ...
                if let (Some(root), Some(_node)) = (&self.root, self.root_node) { root.on_drag(delta); }
            }
            WindowEvent::MouseWheel { delta, .. } => {
                ...
                if let (Some(root), Some(_node)) = (&self.root, self.root_node) {
 root.on_scroll(scroll_y); }
            }
                        ...
                        if let Ok((output, view, mut encoder)) = renderer.begin_frame() {
                            {
                                ...
                                root.paint(renderer, &self.taffy, root_node, is_hovered, &mut _pass, Vec2::zero());
...
impl App {
    ...
    pub fn run(mut self) {
        ...
        let mut runner = RupauiRunner { window: None, renderer: None, taffy: TaffyTree::new(), _layout_map: HashMap::new(), app_name: self.name.clone(), root: self.root, cursor_pos: Vec2::zero(), root_node: None };
