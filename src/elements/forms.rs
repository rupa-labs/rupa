use crate::utils::{Style, generate_id, StyleModifier, Signal, Theme, Color, Accessibility, Attributes, TextAlign, Vec2};
use crate::Component;
...
impl Component for Label {
    ...
    fn paint(&self, renderer: &mut Renderer, taffy: &TaffyTree<()>, node: NodeId, _is_group_hovered: bool, _render_pass: &mut wgpu::RenderPass<'_>, global_pos: Vec2) {
        ...
        renderer.draw_text(&self.text, global_pos.x, global_pos.y, 14.0, [1.0, 1.0, 1.0, 1.0], TextAlign::Left);
    }
    fn on_click(&self) {}
...
impl Component for Checkbox {
    ...
    fn paint(&self, renderer: &mut Renderer, taffy: &TaffyTree<()>, node: NodeId, _is_group_hovered: bool, _render_pass: &mut wgpu::RenderPass<'_>, global_pos: Vec2) {
        ...
        renderer.draw_rect(global_pos.x, global_pos.y, 20.0, 20.0, color, 4.0);
    }
...
impl Component for Radio {
    ...
    fn paint(&self, renderer: &mut Renderer, taffy: &TaffyTree<()>, node: NodeId, _is_group_hovered: bool, _render_pass: &mut wgpu::RenderPass<'_>, global_pos: Vec2) {
        let _layout = taffy.layout(node).unwrap(); renderer.draw_rect(global_pos.x, global_pos.y, 20.0, 20.0, [0.3, 0.3, 0.3, 1.0], 10.0);
        if self.selected.get() { renderer.draw_rect(global_pos.x + 5.0, global_pos.y + 5.0, 10.0, 10.0, [0.39, 0.45, 1.0, 1.0], 5.0); }
    }
...
impl Component for Select {
    ...
    fn paint(&self, renderer: &mut Renderer, taffy: &TaffyTree<()>, node: NodeId, _is_group_hovered: bool, _render_pass: &mut wgpu::RenderPass<'_>, global_pos: Vec2) {
        let layout = taffy.layout(node).unwrap(); renderer.draw_rect(global_pos.x, global_pos.y, layout.size.width, layout.size.height, [0.15, 0.15, 0.15, 1.0], 4.0);
        let text = self.options.get(self.selected_index.get()).cloned().unwrap_or_default(); renderer.draw_text(&text, global_pos.x + 8.0, global_pos.y + 8.0, 14.0, [1.0, 1.0, 1.0, 1.0], TextAlign::Left);
    }
    fn on_click(&self) {}
    fn on_scroll(&self, _: f32) {}
    fn on_drag(&self, _: Vec2) {}
...
impl Component for Switch {
    ...
    fn paint(&self, renderer: &mut Renderer, taffy: &TaffyTree<()>, node: NodeId, _is_group_hovered: bool, _render_pass: &mut wgpu::RenderPass<'_>, global_pos: Vec2) {
        let _layout = taffy.layout(node).unwrap(); let bg_color = if self.on.get() { [0.15, 0.68, 0.37, 1.0] } else { [0.3, 0.3, 0.3, 1.0] }; renderer.draw_rect(global_pos.x, global_pos.y, 40.0, 20.0, bg_color, 10.0);
        let knob_x = if self.on.get() { 22.0 } else { 2.0 }; renderer.draw_rect(global_pos.x + knob_x, global_pos.y + 2.0, 16.0, 16.0, [1.0, 1.0, 1.0, 1.0], 8.0);
    }
...
impl Component for Input {
    ...
    fn paint(&self, renderer: &mut Renderer, taffy: &TaffyTree<()>, node: NodeId, is_group_hovered: bool, _render_pass: &mut wgpu::RenderPass<'_>, global_pos: Vec2) {
        ...
        if let Some(color) = style.background.color.clone() { renderer.draw_rect(global_pos.x, global_pos.y, layout.size.width, layout.size.height, color.to_rgba(), 4.0); }
        let text = self.value.get(); let display_text = if text.is_empty() { &self.placeholder } else { &text };
        renderer.draw_text(display_text, global_pos.x + 8.0, global_pos.y + 8.0, 14.0, [1.0, 1.0, 1.0, 1.0], TextAlign::Left);
    }
    fn on_click(&self) {}
    fn on_scroll(&self, _: f32) {}
    fn on_drag(&self, _: Vec2) {}
