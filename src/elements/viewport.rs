...
// --- LOGIC ---
pub struct ViewportLogic<'a> { pub child: Option<Box<dyn Component + 'a>> }
// --- VIEW ---
pub struct ViewportView { pub core: ViewCore }
// --- COMPONENT ---
pub struct Viewport<'a> { pub id: String, pub logic: ViewportLogic<'a>, pub view: ViewportView }

impl<'a> Viewport<'a> {
    pub fn new() -> Self {
        let mut style = Style::default(); Theme::current().apply_defaults(&mut style);
        Self { id: generate_id(), logic: ViewportLogic { child: None }, view: ViewportView { core: ViewCore::new(style) } }
    }
    pub fn child(mut self, child: Box<dyn Component + 'a>) -> Self { self.logic.child = Some(child); self.view.core.mark_dirty(); self }
}

impl<'a> Stylable for Viewport<'a> { fn get_style_mut(&self) -> RwLockWriteGuard<'_, Style> { self.view.core.get_style_mut() } }

impl<'a> Component for Viewport<'a> {
    fn id(&self) -> &str { &self.id }
    fn children(&self) -> Vec<&dyn Component> { self.logic.child.as_ref().map(|c| c.as_ref() as &dyn Component).into_iter().collect() }
    ...
    fn layout(&self, taffy: &mut TaffyTree<()>, measurer: &dyn TextMeasurer, parent: Option<NodeId>) -> NodeId {
        ...
        if let Some(ref child) = self.logic.child { child.layout(taffy, measurer, Some(node)); }
    ...
    fn paint(&self, renderer: &mut dyn Renderer, taffy: &TaffyTree<()>, node: NodeId, is_group_hovered: bool, global_pos: Vec2) {
        ...
        if let Some(ref child) = self.logic.child { child.paint(renderer, taffy, node, is_group_hovered || style_ref.is_group, global_pos); }
    }
}
