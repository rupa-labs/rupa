...
use crate::utils::TextAlign;
use crate::renderer::batcher::{Batcher, Vertex};
...
impl Renderer {
    pub async fn new(window: Arc<WinitWindow>) -> Self {
        ...
        Self { 
            ...
            camera_offset: Vec2::zero(), camera_zoom: 1.0,
    ...
    pub fn draw_rect(&mut self, x: f32, y: f32, w: f32, h: f32, color: [f32; 4], radius: f32) {
        ...
        let tw = w * self.camera_zoom; 
        let th = h * self.camera_zoom;
        
        let x_norm = (tx / self.size.width as f32) * 2.0 - 1.0; 
        let y_norm = 1.0 - (ty / self.size.height as f32) * 2.0;
        let w_norm = (tw / self.size.width as f32) * 2.0; 
        let h_norm = (th / self.size.height as f32) * 2.0;
        
        let size = [tw, th];
        let radius = radius * self.camera_zoom;
    ...
    pub fn draw_text(&mut self, content: &str, x: f32, y: f32, size: f32, color: [f32; 4], _align: TextAlign) {
        let tx = (x + self.camera_offset.x) * self.camera_zoom; 
        let ty = (y + self.camera_offset.y) * self.camera_zoom;
        ...
        buffer.set_size(&mut self.text_renderer.font_system, Some(self.size.width as f32), Some(self.size.height as f32));
        buffer.set_text(
        ...
        self.text_entries.push(StoredText {
            buffer,
            pos: Vec2::new(tx, ty),
            color,
        });
    ...
    pub fn push_clip(&mut self, x: f32, y: f32, w: f32, h: f32, render_pass: &mut RenderPass<'_>) {
        let x = (x.max(0.0) * self.camera_zoom) as u32; 
        let y = (y.max(0.0) * self.camera_zoom) as u32;
        let w = (w.max(0.0) * self.camera_zoom) as u32; 
        let h = (h.max(0.0) * self.camera_zoom) as u32;
        render_pass.set_scissor_rect(x, y, w.min(self.size.width - x), h.min(self.size.height - y));
    }

    pub fn pop_clip(&mut self, render_pass: &mut RenderPass<'_>) { 
    ...
    pub fn begin_frame(&mut self) -> Result<(SurfaceTexture, TextureView, CommandEncoder), SurfaceError> { 
    ...
    pub fn flush_batches<'a>(&'a mut self, render_pass: &mut RenderPass<'a>) { 
        render_pass.set_pipeline(&self.render_pipeline); 
        render_pass.set_bind_group(0, &self.default_texture.bind_group, &[]); 
        self.batcher.flush(&self.queue, render_pass); 
    }
    ...
    pub fn end_frame(&mut self, output: SurfaceTexture, mut encoder: CommandEncoder) {
        ...
        self.text_renderer.prepare(&self.device, &self.queue, text_areas, self.size.width, self.size.height);
