...
*   **Interface over Wrapping (The Port Mandate)**: Third-party libraries MUST NOT be used directly across multiple modules or exposed in public APIs. Instead, the framework SHALL provide its own **Interface (Port)**, and the third-party library SHALL be encapsulated within an **Implementation (Adapter)**. This ensures that external dependencies can be swapped or replaced with internal Rupa engines without breaking the system.
